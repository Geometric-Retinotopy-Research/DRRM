...
% 
% 
% vis = zeros(size(pt,1),1);
% n = size(Vt,1);
% vis(1:n)=Et.Vertex_vis(:,1);
% plot_mesh(F,pt,'EdgeColor','g','LineWidth',1);
% hold on


% plot_mesh(Ft,Vt,'EdgeColor','r','LineWidth',1);
% alpha(0.2)


%%
% figure
% uv = disk_harmonic_map(F,[V ones(size(V,1),1)]);
% plot_mesh(F,uv);
