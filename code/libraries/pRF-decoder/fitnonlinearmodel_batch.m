...
batchsize=1e3;
for batchloop=1:batchsize:vnum
    
    % check meta file
    metafn = sprintf('prf_%d.meta', batchloop);
    resultfn = sprintf('prf_%d.mat', batchloop);
    if (exist(metafn) || exist(resultfn) )
        fprintf('found meta or result file %s, skipping... \n',metafn );
        continue;
    end
    
    % write meta file
    fid =fopen(metafn, 'w');  fclose(fid);
    
    beginp = batchloop;
    stopp = min(batchloop+batchsize-1, vnum);
    ...
    for p = beginp:stopp        
        ...
        % save result to file
        save(resultfn, 'results0');
    end
    
end

% % Check all results, if not all results are there, just quit, someone will merge
% for batchloop=1:batchsize:vnum
% 
%     resultfn = sprintf('prf_%d.mat', batchloop);
%     if exist(resultfn)
%         continue;
%     else
%         quit(100,"force"); % quite with 100
%     end
% end

% Combine results0, to here, every result will exist
clear results0;
for batchloop=1:batchsize:vnum

    resultfn = sprintf('prf_%d.mat', batchloop);
     
    section_result = load(resultfn);
    
    beginp = batchloop;
    stopp = min(batchloop+batchsize-1, vnum);
    
    for p = beginp:stopp
        results0(p) = section_result.results0(p);
    end    
