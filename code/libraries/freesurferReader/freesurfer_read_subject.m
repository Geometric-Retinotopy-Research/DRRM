...
if strmatch('all',filetype),
	filetype = {'bem','orig','smoothwm','white','pial','inflated','curv','thickness','aparc','xfm','sphere.reg'};
end
...
for f = filetype,
	...
	switch f,
		...
		case {'sulc'}
			...
			% load white surfaces
			file.lh.sulc = fullfile(surfPath,'lh.sulc');
			if exist(file.lh.sulc) == 2,
				lh.sulc = freesurfer_read_sulc(file.lh.sulc);
			else
				msg = sprintf('file does not exist: %s',file.lh.sulc);
				warning(msg);
			end

			file.rh.sulc = fullfile(surfPath,'rh.sulc');
			if exist(file.rh.sulc) == 2,
				rh.sulc = freesurfer_read_sulc(file.rh.sulc);
			else
				msg = sprintf('file does not exist: %s',file.rh.sulc);
				warning(msg);
            end
		...
		case {'sphere.reg'},
			...
			% load sphere
			file.lh.spherereg = fullfile(surfPath,'lh.sphere.reg');
			if exist(file.lh.spherereg) == 2,
				[lh.spherereg.vertices, lh.spherereg.faces] = freesurfer_read_surf(file.lh.spherereg);
			else
				msg = sprintf('file does not exist: %s',file.lh.spherereg);
				warning(msg);
			end

			% load sphere
			file.rh.spherereg = fullfile(surfPath,'rh.sphere.reg');
			if exist(file.rh.spherereg) == 2,
				[rh.spherereg.vertices,rh.spherereg.faces] = freesurfer_read_surf(file.rh.spherereg);
			else
				msg = sprintf('file does not exist: %s',file.rh.spherereg);
				warning(msg);
			end
